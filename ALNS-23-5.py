...
class ALNSSolver:
    ...
    def try_insert_task(self, solution, vessel, task_type, workload, berth):
        ...
        # 步骤2: 调用重新调度函数
        original_start_time = max(TY, TYZ)
    ...
    def reschedule_after_insertion(self, solution, vessel, task_type, berth, workload, original_start_time):
        ...
        next_occupation_start = berth_occupations[occ_index + 1]['start'] if occ_index < len(berth_occupations) - 1 else float('inf')
        ...
        # a) 检查是否与前一个占用发生冲突
        if start_time1 < prev_occupation_end - 1e-6:
